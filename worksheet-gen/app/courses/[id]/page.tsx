  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#121417' }}>
      ...
      <main className="max-w-[1200px] mx-auto px-8 py-8">
        ...
        {/* ── Tab navigation ── */}
        <div
          className="flex gap-7 mb-8 text-sm"
        ...
        {activeTab === 'overview' && (
          <div className="flex gap-8 items-start">
            ...
            {/* Left sidebar — stat mini-cards */}
            <div className="flex flex-col gap-3" style={{ width: 220, flexShrink: 0 }}>
              <StatCard
                value={allWorksheets.length}
                label={allWorksheets.length === 1 ? 'question set' : 'question sets'}
              />
              <StatCard
                value={studentRows.length}
                label={studentRows.length === 1 ? 'student' : 'students'}
              />
              {course.syllabus_code && (
                ...
                >
                  ...
                  <p className="text-2xl font-bold tracking-tight text-white mb-0.5">
                    {course.syllabus_code as string}
                  </p>
                  {syllabusLabel && (
                    <p className="text-xs" style={{ color: '#A8B0BE' }}>{syllabusLabel}</p>
                  )}
                </div>
              )}
            </div>

            {/* Right column — activity stream + question sets */}
                ...
                {sorted.length > 0 ? (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {sorted.length === 1 && (
                      <Link
                        href={createHref}
                        className="group block rounded-xl p-5 transition-all duration-150 hover:border-[#4D528A]"
                        style={{ border: '1.5px dashed #2C2E33' }}
                      >
                        <div
                        ...
                        >
                          <div
                            className="w-7 h-7 rounded-full flex items-center justify-center transition-colors group-hover:bg-[#252830]"
                          ...
                          <span className="text-xs transition-colors group-hover:text-[#9AA0A6]" style={{ color: '#4B5563' }}>
                            Create another set
                          </span>
                        </div>
                      </Link>
                  ...
                  <div
                    ...
                    style={{
                      ...
                      border: '1px dashed rgba(77,82,138,0.28)',
                    }}
                  >
                    <p className="text-sm mb-5" style={{ color: '#A8B0BE' }}>
